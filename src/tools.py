...
__all__ = ["fetch_pubmed_articles", "fetch_clinical_trails", "fetch_medline_plus"]


def fetch_pubmed_articles(search_term: str) -> list[dict[str, Any]]:
    """
    ...
    try:
        ...
        pmids = fetcher.pmids_for_query(search_term, retmax=7)
        ...
        for pmid in pmids:
            ...
            article_data = {
                "title": article.title,
                "content": article.abstract if article.abstract is not None else "",
...
def get_clinical_trails(search_term: str) -> dict[str, Any]:
...
def fetch_clinical_trails(search_term: str) -> list[dict[str, Any]]:
    """
    ...
    for study in output.get("studies", []):
        ...
        extracted_study_data = {
            ...
            + ("primary outcomes: " + " ".join(primary_outcomes) if primary_outcomes else ""),
...
def _fetch_medline_plus_raw(search_term: str) -> ET.Element:
...
def fetch_medline_plus(search_term: str) -> list[dict[str, Any]]:
    """
    ...
    for doc in root.findall(".//document"):
        topic = {"title": "", "url": doc.get("url", ""), "content": ""}
        ...
        for content in doc.findall("content"):
            ...
            if name == "title":
                ...
                topic["content"] = _clean_text(text)
